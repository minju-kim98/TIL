...
int main(){
	...
	while(1){
		...
		printf("%s", prompt);
		scanf("%s", inputArg);
		for(int i = 0; i < strlen(inputArg); i++)
			inputs[idx][i] = inputArg[i];
		idx++;
		if(!strcmp(inputArg, "exit"))	break;
		else if(!strcmp(inputArg, "date") || !strcmp(inputArg, "uptime"))
			system(inputArg);
		else if(!strcmp(inputArg, "ls"))
			system("ls -al");
		else if(!strcmp(inputArg, "log"))
			system("dmesg");
		else if(!strcmp(inputArg, "history")){
			for(int i = 0; i < idx; i++)
				printf("%d %s\n", i+1, inputs[i]);
		}
		else if(!strcmp(inputArg, "hclear")){
			memset(inputs, ' ', sizeof(char));
			idx = 0;
		}
		else if(inputArg[0] == '!'){
			char numinput[100];
			strcpy(numinput, inputArg + 1);
			int num = atoi(numinput);
			char *token = strtok(inputs[num-1], " ");
			system(token);
		}
		else
			printf("ERROR\n");
